...
Pushing a Firefox release tag such as `firefox-v2026.9.10.1` runs the Firefox release workflow, derives the numeric extension version from the tag, creates a GitHub Release titled as a Firefox community build, and attaches a versioned Mozilla-signed Firefox XPI. Chrome remains covered by the regular build workflow but is not published by this fork. Local and manually dispatched builds use version `2026.9.10.1`. Firefox signing uses the `AMO_JWT_ISSUER` and `AMO_JWT_SECRET` GitHub Actions repository secrets.
